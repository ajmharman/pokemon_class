...
class Pokemon:  # defines the pokemon class and their starting attributes based on the inputs.
    ...
    def __repr__(
        self
    ):  # Prints pokemon info when pokemon is called based on pokemon consciousness.
        if self.conscious == True:
            return "This pokemon is called {a} and has reached level {b}. It is of kind '{c}' and has current health {d}. It is conscious.".format(
                a=self.name, b=self.level, c=self.kind, d=self.current_health
            )
        elif self.conscious == False:
            return "This pokemon is called {a} and has reached level {b}. It is of kind '{c}' and has current health {d}. It is unconscious.".format(
                a=self.name, b=self.level, c=self.kind, d=self.current_health
            )

    def lose_health(
        self, damage
    ):  # removes health from a pokemon and knocks them out if they reach 0hp
        ...
        if self.current_health < 0:
            ...
            print(
                "{a} has lost {b} health and has {c} remaining.".format(
                    a=self.name, b=damage, c=self.current_health
                )
            )

    def regain_health(
        self, healing
    ):  # heals the pokemon up to a limit of their max health.
    ...
    def attack(
        self, damaged_pokemon
    ):  # causes damage to a second pokemon based on the attacking and defending kinds.
        attack_modifier = (
            1
        )  # used to adjust damage based on kinds, stays as 1 if kinds are the same.
        ...
        if self.conscious == True:
            ...
            print(
                "{a} attacked {b} and did {c} points of damage.".format(
                    a=self.name, b=damaged_pokemon.name, c=attack_damage
                )
            )
            damaged_pokemon.lose_health(attack_damage)
        else:
            print("{a} cannot attack as they are unconscous.".format(a=self.name))
...
class Trainer:  # defines class of trainers with inputs
    def __init__(
        ...
        self.active_pokemon = self.pokemon_list[
            self.currently_active
        ]  # pulls the active pokemon out of the list using the index of currently_active

    def __repr__(self):  # on calling a trainer, prints the basic information
        return "{a} is a pokemon trainer in possessetion of {b} potions and {c} pokemon. Of those pokemon one is active. {d}".format(
            a=self.name,
            b=self.no_of_potions,
            c=len(self.pokemon_list),
            d=self.pokemon_list[self.currently_active],
        )

    def use_potion(
        self, pokemon_index
    ):  # uses a potion to heal 30 health a stated pokemon
        ...
        if self.no_of_potions > 0:
            if selected_pokemon.conscious:
                ...
                selected_pokemon.revive_pokemon
        else:
            print("You have no potions trainer!")

    def attack_other_trainer(
        self, other_trainer
    ):  # attacks another trainers active pokemon
        print(
            "Trainer {a} attacked trainer {b} using {c} against {d}.".format(
                a=self.name,
                b=other_trainer.name,
                c=self.active_pokemon.name,
                d=other_trainer.active_pokemon.name,
            )
        )
    ...
    def switch_pokemon(
        self, new_pokemon_index
    ):  # switches the active pokemon of this trainer and the index stored as active
